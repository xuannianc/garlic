...
def loss(y_true, y_pred):
    ...
    :return:
    """
    return inside_shrink_quad_score_loss(y_true, y_pred) * config.LAMBDA_INSIDE_SHRINK_QUAD_SCORE_LOSS + \
           inside_end_quads_score_loss_part1(y_true, y_pred) * config.LAMBDA_INSIDE_END_QUADS_SCORE_LOSS + \
           inside_end_quads_score_loss_part2(y_true, y_pred) * config.LAMBDA_INSIDE_END_QUADS_SCORE_LOSS + \
           vertices_coord_loss(y_true, y_pred) * config.LAMBDA_VERTICES_COORD_LOSS


def part1_pos(y_true, y_pred):
    labels1 = y_true[:, :, :, 0]
    logits1 = y_pred[:, :, :, 0]
    # apply sigmoid activation
    predicts1 = tf.nn.sigmoid(logits1)
    beta1 = 1 - tf.reduce_mean(labels1)
    pos = tf.reduce_mean(-1 * beta1 * labels1 * tf.log(predicts1 + config.EPSILON))
    return pos


def part1_neg(y_true, y_pred):
    labels1 = y_true[:, :, :, 0]
    logits1 = y_pred[:, :, :, 0]
    # apply sigmoid activation
    predicts1 = tf.nn.sigmoid(logits1)
    beta1 = 1 - tf.reduce_mean(labels1)
    neg = tf.reduce_mean(-1 * (1 - beta1) * (1 - labels1) * tf.log(1 - predicts1 + config.EPSILON))
    return neg


######################################## inside_shrink_quad_score_loss #############################
def inside_shrink_quad_score_loss(y_true, y_pred):
    labels1 = y_true[:, :, :, 0]
    logits1 = y_pred[:, :, :, 0]
    # apply sigmoid activation
    predicts1 = tf.nn.sigmoid(logits1)
    ...
    # tf.summary.scalar('inside_shrink_quad_score_loss', inside_shrink_quad_score_loss)
    return inside_shrink_quad_score_loss


######################################## inside_end_quad_score_loss ################################
def inside_end_quads_score_loss_part1(y_true, y_pred):
    labels1 = y_true[:, :, :, 0]
    labels2 = y_true[:, :, :, 1:3]
    logits2 = y_pred[:, :, :, 1:3]
    ...
    return inside_end_quads_score_loss_part1


def inside_end_quads_score_loss_part2(y_true, y_pred):
    labels2 = y_true[:, :, :, 1:3]
    logits2 = y_pred[:, :, :, 1:3]
    predicts2 = tf.nn.sigmoid(logits2)
    ...
    # tf.summary.scalar('inside_end_quads_score_loss_part1', inside_end_quads_score_loss_part1)
    # tf.summary.scalar('inside_end_quads_score_loss_part2', inside_end_quads_score_loss_part2)
    return inside_end_quads_score_loss_part2


####################################### vertices_coord_loss ###############################################
def vertices_coord_loss(y_true, y_pred):
    labels2 = y_true[:, :, :, 1:3]
    inside_end_quads_mask = tf.cast(tf.equal(labels2[:, :, :, 0], 1), tf.float32)
    labels3 = y_true[:, :, :, 3:]
    predicts3 = y_pred[:, :, :, 3:]
    ...
    # tf.summary.scalar('vertices_coord_loss', vertices_coord_loss)
    return vertices_coord_loss
